...
void runThreadingDemo()
{
    ...
    for (int i = 1; i <= 4; i++)          // 4 threads: more than the required minimum of 3
    {
...
void incrementUnsafe(int times)
{
    for (int i = 0; i < times; i++)
    {
        ...
        std::this_thread::yield();  // let another thread jump in here -- this is what causes the race
        unsafeCounter = temp + 1;   // write (may overwrite someone else's update!)
    }
}
